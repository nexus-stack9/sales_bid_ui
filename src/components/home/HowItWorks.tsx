...
import { ArrowRight, UserRound, Search, DollarSign, PackageCheck } from 'lucide-react';
...
const steps = [
  {
    ...
    description: 'Sign up for free and get verified to start bidding on premium inventory.',
    icon: <UserRound className="h-6 w-6 text-primary" />
  },
  {
    number: '02',
    title: 'Browse auctions',
    description: 'Explore thousands of auctions from retailers and manufacturers.',
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className={`text-center mb-16 transition-all duration-700 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <p className="mt-3 text-lg text-muted-foreground max-w-2xl mx-auto">
            Simple steps to start bidding and winning auctions in our marketplace
          </p>
        </div>
        
        <div className="relative">
          <div className="hidden lg:block absolute top-32 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-primary/20 to-transparent" />
